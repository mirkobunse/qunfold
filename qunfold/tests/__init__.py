...
class TestMethods(TestCase):
  def test_methods(self):
    ...
    for _ in range(10):
      ...
      p_kmmf = qunfold.KMM('fast').fit(X_trn, y_trn).predict(X_tst)
      p_kmmrff = qunfold.KMM('rff').fit(X_trn, y_trn).predict(X_tst)
      ...
      print(
        ...
        f"     p_kmmf = {p_kmmf}",
        f"             {p_kmmf.nit} it.; {p_kmmf.message}",
        f"     p_kmmrff = {p_kmmrff}",
        f"             {p_kmmrff.nit} it.; {p_kmmrff.message}",
        f"     p_tst = {p_tst}",
        sep = "\n",
        end = "\n"*2
      )
